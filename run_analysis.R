...
if (!file.exists(datadir)){
    message(paste("The data folder:", datadir, "does not exist!", sep=" "))
} else {
...
#This the code will combine 2 datasets
...
extdata <- cbind(alldata["subject"], alldata["label"])
for (m in extnames){
    extdata <- cbind(extdata, alldata[m])
}
...
mdata <- melt(extdata, id=names(extdata)[1:2], measure.vars=extnames)
cdata <- tapply(mdata$value, list(mdata$subject, mdata$label, mdata$variable), mean)
for (i in c(1:30)){
    t <- cbind(1,rownames(cdata[i,,]),cdata[i,,])
    if (i>1){
